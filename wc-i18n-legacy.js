...
window.WCI18n = function (locales) {
  return {
    ...
    attached: function () {
      ...
      if (!Array.isArray(_instances[componentName])) _instances[componentName] = [];
    ...
    __fetchLocaleStrings: function (newLang) {
      return new Promise(function (resolve) {
        ...
        if (_strings[componentName] && _strings[componentName][newLang]) return resolve(_strings[componentName][newLang]);

        // Compute the destination of the locales
        var url = _computeLocaleURI(`${_componentPaths[componentName]}/locales`, componentName, newLang);
        ...
        _strings[componentName][newLang] = promise;
        ...
        return resolve(promise.then(function (locales) {
          ...
          _strings[componentName][newLang] = locales;
          return locales;
        })
          .catch(function (err) {
          // Unset the promise to allow refetching of the data if
          // possible
            _strings[componentName][newLang] = null;
            throw err;
          }));
      }.bind(this));
    },
    ...
    __updateLanguage: function (newLang, oldLang) {
      ...
      this.__fetchLocaleStrings(newLang)
        .then(function (locales) {
          var fxnKey = componentName + '.' + newLang;
